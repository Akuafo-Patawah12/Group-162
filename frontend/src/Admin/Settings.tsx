...
const Settings = () => {
  const { data: userSettings, isLoading } = useGetSettingsQuery();
  const [updateSetting] = useUpdateSettingMutation();

  const handleChange = (setting: UserSetting, newValue: string | boolean) => {
    updateSetting({ ...setting, value: newValue });
  };
                  ...
                  {setting.type === "toggle" ? (
                    <label className="inline-flex relative items-center cursor-pointer">
                      <input
                        ...
                        onChange={() =>
                          handleChange(setting, !(setting.value as boolean))
                        }
                      />
